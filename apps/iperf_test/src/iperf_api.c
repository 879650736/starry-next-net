...
{
    printf("ipt->mode: %d, ipt->protocol->id: %d, has_tcpinfo_retransmits(): %d\n",
           ipt->mode, ipt->protocol->id, has_tcpinfo_retransmits());
...
{
    ipt->role = role;
    printf("ipt->reverse: %d, ipt->bidirectional: %d, role: %c\n", ipt->reverse, ipt->bidirectional, role);
    if (!ipt->reverse) {
...
iperf_set_test_server_hostname(struct iperf_test *ipt, const char *server_hostname)
{
    ...
    printf("iperf_set_test_server_hostname: %s\n", ipt->server_hostname);
}
...
{
    printf("iperf_parse_arguments: argc %d\n", argc);
...
iperf_defaults(struct iperf_test *testp)
{
    ...
    printf("Setting defaults for test\n");
    testp->omit = OMIT;
...
{
    printf("iperf_stats_callback called\n");
    struct iperf_stream *sp;
...
{
    printf("iperf_reporter_callback called\n");
    printf("test->state: %d\n", test->state);
    switch (test->state) {
        case TEST_RUNNING:
        case STREAM_RUNNING:
...
{
    printf("beginning iperf_create_pidfile\n");
    printf("test->pidfile: %s\n", test->pidfile);
    if (test->pidfile) {
        printf("test->pidfile is not NULL\n");        
        int fd;
        char buf[8];
