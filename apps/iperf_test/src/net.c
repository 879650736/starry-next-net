...
timeout_connect(int s, const struct sockaddr *name, socklen_t namelen,
    int timeout)
{
    printf("timeout_connect: s=%d, name=%p, namelen=%d, timeout=%d\n", s, name, namelen, timeout);
	struct pollfd pfd;
	socklen_t optlen;
	int flags, optval;
	int ret;
    ...
    int connect_ret = connect(s, name, namelen);
    printf("timeout_connect: connect returned %d, errno=%d\n", connect_ret, errno);
	if ((connect_ret) != 0 && errno == EINPROGRESS) {
        printf("timeout_connect: connection in progress\n");
		pfd.fd = s;
		pfd.events = POLLOUT;
...
create_socket(int domain, int proto, const char *local, const char *bind_dev, int local_port, const char *server, int port, struct addrinfo **server_res_out)
{
    ...
    printf("create_socket: domain=%d, proto=%d, local=%s, bind_dev=%s, local_port=%d, server=%s, port=%d\n",
           domain, proto, local , bind_dev , local_port, server , port);
...
{
    printf("netdial: domain=%d, proto=%d, local=%s, bind_dev=%s, local_port=%d, server=%s, port=%d, timeout=%d\n",
           domain, proto, local , bind_dev , local_port, server , port, timeout);
    struct addrinfo *server_res = NULL;
    int s, saved_errno;

    s = create_socket(domain, proto, local, bind_dev, local_port, server, port, &server_res);
    printf("netdial: created socket %d\n", s);
    if (s < 0) {
      return -1;
    }
...
setnonblocking(int fd, int nonblocking)
{
    ...
    printf("setnonblocking: fd=%d, nonblocking=%d, flags=%d\n", fd, nonblocking, flags);
    if (nonblocking)
	    newflags = flags | (int) O_NONBLOCK;
    else
	    newflags = flags & ~((int) O_NONBLOCK);
    printf("setnonblocking: newflags=%d\n", newflags);
    if (newflags != flags)
